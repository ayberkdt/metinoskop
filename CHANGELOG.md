...
- İki gold yeniden kalibre edildi. `provide-kalkisi` çıktısı `operatörün görünürlüğünü artırıyor` diyordu; bu Türkçede operatörün başkalarınca görülmesi diye okunuyor ve kaynağın anlamını tersine çeviriyordu. Yeni çıktı `operatörün daha iyi görmesini sağlıyor`; `sağlamak` doğal olduğu yerde geri döndü, çünkü amaç sözcüğü sökmek değil `provide` için varsayılan olmasını engellemek. `elde-etmek-olcum` çıktısı ise kalkıyı kaldırırken yeni bir tuhaf eşdizim üretiyordu (`hata oranı iyileşti`); artık `hata oranı %20 azaldı`.
- `fayda üretmek` sert adaydan (Sınıf A) bağlamsal çekirdeğe (Sınıf B) indirildi: iktisat, kamu politikası ve kurumsal metinlerde `toplumsal fayda üretmek` doğaldır ve `karar gerçekleştirmek` ile aynı güven düzeyinde yanlış değildir.
- Kritik canlı sete `provide` çifti (`provide-kalkisi` ve `erisim-saglamak-korunur`) eklendi; yerine kayıtlı çıktı denetiminin güçlü koruduğu iki mekanik vaka (`ve-zinciri`, `sahip-olmak-kalkisi`) çıkarıldı. Liste 36 vakada kaldı, haftalık koşu maliyeti artmadı.
- `evals/critical-cases.txt` üst sınırı 30'dan 36'ya çıkarıldı ve altı sözcüksel kalkı vakası eklendi: üç dönüşüm–koruma çifti (`framework-kalkisi` / `referans-cercevesi-korunur`, `campaign-kalkisi` / `reklam-kampanyasi-korunur`, `address-kalkisi` / `bellek-adresleme-korunur`). Ayrım gerçek modelde çift olarak sınanır; haftalık koşunun maliyeti buna bağlı olarak artar.
- `-yordu` ve `-mıştı` tanımları düzeltildi. `-yordu` olayın sınırsız olduğunu değil, geçmişteki bir referans noktasına göre sürerken sunulduğunu bildirir; `Pompa iki dakika boyunca çalışıyordu` cümlesinde süre sınırlıdır. `-mıştı` için referans noktası aynı cümlede bulunmak zorunda değildir; önceki cümlelerden ya da paragrafın zaman çizgisinden gelebilir.
- Dönüşüm vakalarındaki kesin cümle sayısı yasağı yumuşatıldı: vakanın kendi talebi belirli bir yapı dayatıyorsa `- cümle: = N` yazılabiliyor. Yasak yalnızca varsayılan gibi yazılmış çıplak `- cümle: N` biçimi için geçerli.
- `çeviri gölgesi` terimi `çeviri kokusu` oldu ve `kaynak dil gölgesi` ile birleştirildi; aynı kavram iki adla anılıyordu. `references/turkce-ritim-ve-ceviri-golgesi.md` dosyası `references/turkce-ritim-ve-ceviri-kokusu.md`, `references/esdizim-ve-istem.md` dosyası `references/sozcuk-birlesimleri.md` adını aldı; `SKILL.md`'deki bölüm başlığı `## Sözcük birleşimleri` oldu. `edat gölgesi` `edat izi`, `kılavuz gölgesi` `kılavuz kokusu` olarak yazılıyor. Stil denetimindeki `ceviri_golgesi` ölçü anahtarı `ceviri_kokusu` oldu. İçerik ve kurallar aynı; CHANGELOG'un yayımlanmış sürüm kayıtlarına dokunulmadı.
- README kendi kurallarından bir kez daha geçirildi: aynı terim tanıtma kalıbı dört kez yinelenmişti (işlev tekrarı), üç cümlede `ve` zinciri vardı ve bir ölçü dökümü cümle gibi yazılmıştı. Düzyazıda çeviri kokusu bulgusu sıfıra indi.
...
- Sözcüksel kalkı: bir sözcüğün Türkçe karşılığı doğru olsa bile kaynak dildeki birleşme tercihinin Türkçeye taşınması. `references/sozcuk-birlesimleri.md` üç sınıflı karar sistemi (birleşim düzeyinde sert aday, yüksek riskli çekirdek, alan terimi), kanıt hiyerarşisi, karar ağacı, on dokuz İngilizce çekirdek için karar rehberi, `framework` ve `campaign` anlam ayrımları, sıfat ve zarf eşdizimleri, İngilizce geri kurma testi ile alan jargonu ve terminoloji kapısı bölümleriyle genişletildi. `SKILL.md`'ye tek paragraflık ilke eklendi. İngilizce kök → tek Türkçe karşılık tablosu kurulması açıkça yasaklandı.
- On sekiz yeni eval vakası: on sözcüksel kalkı dönüşümü (`framework`, `campaign`, `capture`, `drive`, `enable`, `address`, `provide`, `experience`, `ortaya koymak` yığını, ölçüm varken `elde etmek`) ve sekiz alan terimi koruması (fizik `referans çerçevesi`, akademik `kuramsal çerçeve`, yazılım `framework`, reklam ve yardım kampanyası, havacılık `test kampanyası`, `belleği adreslemek`, `hipotezi desteklemek`).
- İki yeni koruma vakası: `davranisi-yakalamak-alan-terimi-korunur` (modelleme jargonunda `davranışı yakalamak`) ve `erisim-saglamak-korunur` (`erişim`, `bağlantı`, `güvenliği sağlamak` doğal eşdizimleri). Referans ikisini de kabul ediyordu ama davranışsal sözleşmede karşılıkları yoktu.
- Stil denetimine `sozcuksel_kalki` inceleme sinyali: yalnızca birleşim düzeyinde aday gösterir, tek sözcük işaretlemez. Öz sınama altı adayı ve sekiz yanlış pozitifi korur.

- Şimdiki zaman ve söylemsel yakınlık: `-yor` yalnızca o anda süren eylemi değil, mevcut sistem davranışını, güncel durumu, okurun önündeki belgenin işlevini ve o an tartışılan bulgunun yorumunu da bildirir. Yeni `geniş zaman aşımı` kavramı güncel söylemi zamansız tanım gibi kuran cümleleri adlandırıyor; `bu sonuç` ile `bu tür sonuçlar` arasındaki birim–tür ayrımı kararın çoğunu veriyor. `-maktadır` ile `-yor`, güncel yasak ile normatif kural, `-yor` doygunluğu ve iki seçenek testi aynı bölümde. `references/zamansal-ankraj-ve-rapor-kipi.md` içindeki küçük `-yor` bölümü bu bölümle değiştirildi; `SKILL.md`'ye tek paragraflık ilke eklendi.
- On yeni eval vakası: deponun güncel davranışı, tek bir sonucun güncel yorumu, şekil ve tablonun güncel işlevi, gözlenen sonuç ile güncel yorum ayrımı, projenin bugünkü durumu, bürokratik şimdiki zaman, aynı paragrafta üç zamansal işlev, yerel hata ile hata sınıfı ayrımı; karşı tarafta genel yöntem gerekliliği ve normatif kural korunur.
- Stil denetimine `genis_zaman_katiligi` inceleme sinyali: güncel gönderge ile geniş zamanlı betimleyici yüklemin birleşimi. Bağlamsal; normatif kural, genel bilimsel önerme, algoritma tanımı ve yinelenen takvim kuralı işaretlenmez, öz sınama bunu korur.
...
- Çeviri kokusu ve Türkçe ritim kuralları: dil bilgisi doğru olduğu hâlde İngilizce iskelet taşıyan cümleyi tanıyan "çeviri kokusu" kavramı; "ilişkiyi yeniden kur, sözcükleri değil" ilkesi; bilgi yapısının (konu, odak, verilmiş ve yeni bilgi) sözcük sırasını belirlemesi; açık özne denetimi ve özne düşürmenin kural değil kaynak olması; yan cümle mimarisi denetimi (çekimli cümle zinciri, sıralama ile alt sıralama ayrımı, fiilimsi dengesi, uzak yüklem); Türkçe kaynak geri kazanımı listesi ve geri kazanılabilirlik testi; yerli alternatif üretimi ve geri çeviri testi; daha güçlü fiil uydurmama ve terim koruma kuralı; aşırı düzeltme koruması ve türe göre ritim.
- `references/turkce-ritim-ve-ceviri-kokusu.md`: kokunun yirmiyi aşkın yüzü için katalog ve zorlu önce/sonra örnekleri (özne fazlası, `bu + söylem adı` paketi, `bir`, `sahip olmak`, `var/yok`, `olan` zinciri, çerçeve yığını, soyut ad ve hafif yüklem, eksilti, uzun cümle ritmi, iyelik zinciri, küçük kalkı aileleri); iyi Türkçenin eğilimleri; paragraf ve belge düzeyinde koku; türlere göre davranış; dokunulmayacak karşı örnekler ve aşırı düzeltme belirtileri.
- Çalışma yönteminde altıncı denetim geçişi (F. Çeviri kokusu: özne, yan cümle, niteleme, çerçeve, iyelik ve varlık, bilgi yapısı, yerli kaynak ve aşırı düzeltme denetimi); ses ve akış denetimine "Türkçe düşünülmüş gibi okunuyor mu?" sorusu.
...
- Stil denetimi çeviri kokusu ölçüleri: `sahip olmak`, varlık kalıbı ve çeviri kalkısı aileleri (bağlamsal); tek cümlede çerçeve yığını, `olan` zinciri, `bir` fazlası, `ve` zinciri, fiilimsi yığını ve iyelik zinciri; ardışık cümlelerde aynı özne; paragraf ve belge düzeyinde söylem belirteci ve `bu + söylem adı` yoğunluğu; 100 sözcük başına `bir`. Hepsi inceleme bulgusudur; tek `bir`, `olan`, `ve`, `açısından` işaretlenmez, paralel `açısından` çifti yığın sayılmaz. `--source` ile çıktıda yeni beliren koku bulguları uyarı olarak listelenir. Öz sınama yerli metinde sıfır yanlış pozitif ister.
- Kayıtlı çıktı regresyonu çeviri kokusu ölçülerini `## Yapısal beklenti` satırı olarak kabul eder (`sahip olmak`, `varlık kalıbı`, `çerçeve yığını`, `olan zinciri`, `tekrarlanan cümle başlangıcı` (önceki adıyla `ardışık özne`), `ve zinciri`, `fiilimsi yığını`, `iyelik zinciri`, `bağlaçla başlayan cümle`, `bir / 100 sözcük`); eval çalıştırıcısının öz sınaması `sahip olmak` vakasını da kapsar.
- Doğrulayıcı yeni referans dosyasını, SKILL.md'deki çeviri kokusu ilkelerini, yirmi dört vakanın kapsam işaretlerini, en az otuz kayıtlı çıktıyı ve korunur vakalarında kayıtlı çıktının kaynağa eşit olmasını denetler.
- Epistemik mimari kuralları: epistemik kaynak ayrımı (gözlem, ölçüm, aktarım, alıntı, çıkarım, tahmin, öngörü, beklenti, plan, varsayım, yaygın kabul, yazar ve kurum yorumu, çözümsüz) ve "bilginin kaynağını düzenleyip yok etme" ilkesi; edilgen çatı için altı soruluk karar çerçevesi ve aktörlük koruma; gözlem–ilişki–çıkarım–nedensel iddia düzeyleri; aktarım ve atıf mesafesi; çekince ile pekiştirici ayrımı ve kiplik yığını; zaman–görünüş–kip bakış açısı denetimi; belirsizliğin kapsamı; bölümler arası kesinlik kayması; öneri–plan–karar–uygulama statü zinciri; olumsuz kanıt koruması. `references/kanit-ve-kesinlik.md` katalog, karşı örnek ve aşırı düzeltme belirtileriyle eklendi.
- Sözcük uyumu kuralları: sözcük uyumu için beş soru; eşdizim denetimi ve eşdizim ile klişe ayrımı; fiil istemi ve hâl çerçevesi; ilgeç seçimi ve edat izi; hafif fiil ve genel fiil; yüklem kesinliği; sıfat–ad ve zarf–fiil uyumu; sözcük zincirleri, gönderge zinciri ve eş anlamlı kayması; teknik terim ve ödünç sözcük koruması; türe göre eşdizim. `references/sozcuk-birlesimleri.md` eklendi.
...
- `scripts/behavioral-regression.py` ve `.github/workflows/behavioral.yml`: `SKILL.md` ve referansları gerçek modele verip `evals/critical-cases.txt` içindeki yirmi kritik vakayı (beş epistemik, beş çeviri kokusu, beş yapı, beş doğal metni koruma) taze üreten, deterministik denetim ve model hakemiyle puanlayan davranışsal regresyon; haftalık ve elle tetiklenir, `--dry-run` planı API çağrısı yapmadan yazdırır.
- `SKILL.md` sıkıştırıldı: 83 KB'den yaklaşık 50 KB'ye; çeviri kokusu, epistemik mimari, sözcük uyumu ve tutarlılık bölümleri "ne zaman fark et → hangi kararı ver → hangi referansı oku → neyi asla bozma" düzeyine indirildi (koku sekiz paragraftan üç ilkeye), kalıp katalogları referanslara bırakıldı, denetim geçişleri tek satıra indi. Doğrulayıcı artık 400 satır, 60 KB ve 1.200 baytlık `description` sınırı koyar.
...
- Skill açıklaması çeviri kokusu kapsamını ve "çeviri gibi durmasın", "Türkçe düşünülmüş gibi olsun" tetikleyicilerini içerir; standart müdahale düzeyi çeviri kokusunu kapsar; sorun kümeleri ve referans yönlendirmesi yeni dosyayı gösterir.
...
- `evals/README.md` çeviri kokusu vaka grubunu ve üç yeni hakem ölçütünü (çeviri kokusu, Türkçe kaynak kullanımı, koku avında aşırı düzeltme) tanımlar.
