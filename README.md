...
Türkçe metinlerdeki mekanik yapay zekâ ritmini, basmakalıp geçişleri, gereksiz resmiyeti, pazarlama cilasını ve çeviri kokusunu azaltan taşınabilir bir agent skill.
...
Çalışma zamanında gereken ana dosya `SKILL.md` dosyasıdır. `references/` klasörü ayrıntılı Türkçe örüntü ve akıcılık rehberlerini, `agents/openai.yaml` ise destekleyen istemciler için arayüz metadata'sını içerir.
...
Anlatı yapısının, olay sırasının veya bakış açısının yeniden kurulması ayrı bir anlatı editörlüğü görevidir.
...
3. Kullanıcının istediği müdahale düzeyinde yeniden yazar.
...
Hata örneklerini ve geliştirme önerilerini [GitHub Issues](https://github.com/ayberkdt/metinoskop/issues) üzerinden paylaşabilirsiniz. Davranış değişikliği yapan pull request'lerde `SKILL.md` ile README açıklamalarının birbiriyle uyumlu kalması gerekir.
