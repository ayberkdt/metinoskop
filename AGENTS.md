...
- `scripts/style-lint.py`: Model çıktısındaki şüpheli retorik ve yapısal örüntüleri işlev ailesine göre işaretler; tek başına reddetmez, `--source` ile yeni eklenen kalıpları sert/bağlamsal ayrımıyla listeler. Çeviri gölgesi ölçülerini (`sahip olmak`, varlık kalıbı, çerçeve yığını, `olan` zinciri, ardışık özne, `ve` zinciri, fiilimsi yığını, iyelik zinciri, bağlaçla başlayan cümle, `bir` yoğunluğu) yalnızca inceleme bulgusu olarak raporlar.
